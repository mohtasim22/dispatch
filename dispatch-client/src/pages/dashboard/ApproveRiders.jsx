...
const ApproveRiders = () => {
  ...
  const { data: riders = [], isLoading } = useQuery({
    ...
    queryFn: async () =>
      (await axiosSecure.get("/riders?status=pending")).data.data,
  });

  const decide = useMutation({
    mutationFn: ({ id, action }) =>
      axiosSecure.patch(`/riders/${id}/${action}`),
    onSuccess: (_res, { action }) => {
      queryClient.invalidateQueries({ queryKey: ["pending-riders"] });
      Swal.fire({
        icon: "success",
        title: action === "approve" ? "Rider approved" : "Rejected",
        timer: 1200,
          ...
          {riders.map((r) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-amber-500/15 text-amber-300">
                  pending
                </span>
              </div>
              ...
              {r.licenseUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-block mt-2 text-sm text-teal-300 font-semibold hover:underline"
                >
              ...
              <div className="mt-4 flex gap-2">
                <button
                  onClick={() =>
                    decide.mutate({ id: r._id, action: "approve" })
                  }
